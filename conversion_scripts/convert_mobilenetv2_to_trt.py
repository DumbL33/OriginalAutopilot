...
class MobileNetV2Steering(nn.Module):
    """
    MobileNetV2 pretrained on ImageNet for steering prediction
    """
    def __init__(self, pretrained=False):
        super(MobileNetV2Steering, self).__init__()
        ...
        # X prediction (left/right steering)
        self.x_predictor = nn.Linear(128, 1)
        
        # Action prediction (forward/stop/backward)
...
def export_pytorch_to_onnx(pytorch_model_path, onnx_path, device='cuda'):
    print("="*60)
    print("STEP 1: Exporting MobileNetV2 RGB to ONNX")
    print("="*60)
    
    # Load PyTorch model - USE YOUR MODEL CLASS
    model = MobileNetV2Steering(pretrained=False).to(device)
...
if __name__ == "__main__":
    # Configuration
    PYTORCH_MODEL = "../models/pytorch_models/best_model_park_mobilenet.pth"
    ONNX_MODEL = "../models/tensorrt_mobilenet_models/best_model_park_mobilenet.onnx"
    TENSORRT_ENGINE = "../models/tensorrt_mobilenet_park_models/best_model_park_mobilenet_fp16.trt"
