...
class SteeringModel_MobileNetV2:
    """
    ...
    def __init__(self, engine_path, verbose=True):
        ...
        if self.verbose:
            print(f"✓ Parking steering model loaded: {self.engine_path.name}")
            print(f"  Input: RGB images (not masks)")
    ...
    def preprocess(self, rgb_image):
        """
        Preprocess RGB camera image for model input.
        
        Args:
            rgb_image: numpy array (H, W, 3) - RGB camera image
    ...
    def predict(self, rgb_image):
        ...
        Args:
            rgb_image: numpy array - raw RGB camera image (any size)
        
        Returns:
            x: float - steering in [-1, 1]
            action: float - action in [-1, 1]
        """
        preprocessed = self.preprocess(rgb_image)
