"""
TensorRT Steering Model Inference Module
File: steering_inference.py

Usage:
    from steering_inference import SteeringModel
    
    model = SteeringModel('path/to/steering_model_fp16.trt')
    xy, action = model.predict(mask)
"""
...
class SteeringModel:
    ...
    Predicts (x, y) coordinates and action from lane masks.
    """
    ...
    def __init__(self, engine_path, verbose=True):
        ...
        if self.verbose:
            ...
            print(f"  Outputs: xy (2 values), action (1 value)")
    ...
    def infer(self, input_data):
        ...
        Returns:
            xy: numpy array (2,) - relative coordinates in [-1, 1]
        ...
        return xy_output, float(action_output)
    ...
    def predict(self, mask):
        ...
        Returns:
            xy: numpy array (2,) - [relative_x, relative_y] in range [-1, 1]
                relative_x: -1 (left) to 1 (right)
                relative_y: -1 (close) to 1 (far)
        ...
        xy, action = self.infer(preprocessed)
        return xy, action
